...
def plot_rts(df):
    ...
    results.plot(ax=ax,kind='bar',grid=False)
...
def plot_ems_and_target(df, trial):
    ...
    targ_x = np.ones(len(t))*df['targ_x'][trial]
    targ_y = np.ones(len(t))*df['targ_y'][trial]

    plt.figure()

    # *** YOUR CODE HERE ***
    plt.plot(t, h, 'r', t, v, 'g',t,targ_x,'r',t,targ_y,'g')
    plt.title("Eye Movements for Trial "+ str(trial))
    plt.xlabel('Time (ms)')
    plt.ylabel('Position (degrees visula angle)')
...
def get_rate(spk_times, start, stop):
    ...
    count = np.sum((spk_times >= start) & (spk_times < stop ))
    rate = count*1000.0/(stop-start)
...
if __name__ == "__main__":
    ...
    add_info(df)
    plot_rts(df)
    plot_ems_and_target(df, 213)
    add_aligned_rates(df, 'stimon' ,100, 200)
    print df.pivot_table(values='rates_stimon_100_200', index='targ',columns='targ_ecc')
