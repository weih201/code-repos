...
import inversionCount.mergeCount

object inversion {

  def main(args: Array[String]): Unit = {
    
    val test = List(4, 80, 70, 23, 9, 60, 68, 27, 66, 78, 12, 40, 52, 53,
    ...
    var nums = Source.fromFile("./data/IntegerArray.txt" ).getLines.map(_.toInt).toList
      
//     for(num<-nums)println(num)
     println(nums.length)
     
     println(new mergeCount().mnvcount((x: Int, y: Int) => x < y)(test,0))
     println(new mergeCount().mnvcount((x: Int, y: Int) => x < y)(nums,0))
  }
